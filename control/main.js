  ...
  loop = setInterval(function () {
    ...
    switch (element.name) {
      case "r":
        ...
        var audio = new Audio("public/assets/sounds/coin.wav")
        audio.play()
...
function check() {
  if (victory) {
    ...
    if (win) {
      ...
      if (level < 15) {
        ...
        var audio = new Audio(
          Math.random() > 0.5
            ? "public/assets/sounds/win1.wav"
            : "public/assets/sounds/win2.wav"
        )
        audio.play()
  ...
  if (Player.health <= 0) {
    restart()
    console.log("Defeat!")
    var audio = new Audio("public/assets/sounds/dead.wav")
    audio.play()
  }
}
...
function loadMap(skin) {
  ...
  fetch(`public/assets/levels/${level}/hint.txt`)
  ...
  // Load level layout
  fetch(`public/assets/levels/${level}/level.txt`)
    .then((response) => response.text())
    .then((responseText) => {
      victory = false
      Player.constructor()
...
function changeLevel() {
  ...
  if (levelPopup !== null && level !== levelPopup) {
    fetch(`public/assets/levels/${levelPopup}/level.txt`)
      .then((response) => {
        if (!response.ok) {
...
function showPopup() {
  fetch(`public/assets/levels/${level}/popup.txt`)
    .then((response) => response.text())
    .then((responseText) => {
